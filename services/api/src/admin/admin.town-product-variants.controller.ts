...
export class AdminTownProductVariantsController {
  ...
  // PUT /api/v1/admin/town-products/:id/variants  (replace all)
  @Put(':id/variants')
  async replaceAll(
    ...
    if (!Array.isArray(variants)) throw new BadRequestException('variants must be an array');

    for (const v of variants) {
      const label = String(v?.label ?? '').trim();
      if (!label) throw new BadRequestException('variant.label is required');
      if (v.unitPrice === null || v.unitPrice === undefined || v.unitPrice === '')
        throw new BadRequestException(`unitPrice is required for ${label}`);
