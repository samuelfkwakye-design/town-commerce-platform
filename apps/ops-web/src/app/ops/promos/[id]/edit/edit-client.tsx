...
export default function EditPromoClient() {
  ...
  auth: true,
        ...
        if (
          me.role === "TOWN_SUPER_ADMIN" &&
          (!me.townId || data.townId !== me.townId)
        ) {
          setAccessDenied(true);
          return;
        }
        ...
        setExpiresAt(toDateTimeLocalValue(data.expiresAt));
      } catch (err: any) {
        if (!cancelled) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (!admin || !promo) {
      setError("Promo data is not loaded.");
      return;
    }

    if (isWarehouse) {
      setError("You do not have permission to edit promos.");
      return;
    }

    if (isTownSuper && promo.townId !== admin.townId) {
      setError("You do not have permission to edit this promo.");
      return;
    }
    ...
    try {
      ...
      if (isGlobal) {
        payload.townId = townId.trim() ? townId.trim() : null;
      } else if (isTownSuper) {
        if (!admin.townId) {
    ...
    <main style={{ maxWidth: 820, margin: "0 auto", padding: 24 }}>
      ...
      >
        ...
        <p style={{ marginTop: 0, color: "#666", marginBottom: 24 }}>
          Update an existing promo code safely within role scope.
        </p>

        {bootstrapping || loadingPromo ? (
          <div style={noticeStyle}>Loading promo access...</div>
        ) : accessDenied ? (
          <div style={errorBoxStyle}>
            You do not have permission to edit this promo.
          </div>
        ) : !promo ? (
          <div style={errorBoxStyle}>Promo not found.</div>
        ) : (
          <form onSubmit={onSubmit}>
            ...
            >
              <button
                type="submit"
                disabled={saving || bootstrapping || loadingPromo}
                style={{
                  padding: "10px 16px",
                  borderRadius: 10,
                  ...
                  fontWeight: 700,
                  cursor:
                    saving || bootstrapping || loadingPromo
                      ? "not-allowed"
                      : "pointer",
                  opacity: saving || bootstrapping || loadingPromo ? 0.7 : 1,
                }}
              >
